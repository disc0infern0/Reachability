...
extension Reachability {
    /// 2 Asynchronous ways to get a session response, the method chosen depending on  what platform tools are available.
    ...
    internal static func collectResponse(session: URLSession, request: URLRequest) async throws (URLError) -> URLResponse {
        do {
    ...
    /// Synchronous  method to get a session response based on what tools are available.
    ///  the legacy method uses unchecked Sendable  to allow modification of variables soutside the completion handler (which cannot throw)
    internal static func collectSynchronousResponse(session: URLSession, request: URLRequest)  throws (URLError) -> URLResponse {
        do {
                ...
                return try legacyGet(session: session, request: request)
...
/// Compatibility helper for platforms/OS versions where URLSession.data(for:) isn't available,
    ...
    @available(iOS 13.0, macOS 10.15, tvOS 13.0, watchOS 6.0, *)
    private static func dataTaskCompat(session: URLSession, request: URLRequest) async throws -> URLResponse {
    ...
    nonisolated
    private static func legacyGet(session: URLSession, request: URLRequest) throws -> URLResponse { //}, completion: @Sendable @escaping (Result<URLResponse, URLError>) -> Void) {
